// Define alphabet letters that computer can pick from
    var psychicChoices = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m",
...
// Have computer pick a letter and store it in letterToBeGuessed
    var letterToBeGuessed = psychicChoices[Math.floor(Math.random() * psychicChoices.length)];
...
// the alphabet, and not any non-usable character
    if (guessesSoFar.indexOf(userGuess) < 0 && psychicChoices.indexOf(userGuess) >= 0) {
...
// also have the computer make a new random pick
    if (letterToBeGuessed == userGuess) {
        ...
        guessesLeft = 10;
        guessesSoFar = [];
        letterToBeGuessed = psychicChoices[Math.floor(Math.random() * psychicChoices.length)];
        console.log("Wins: " + wins + " Losses: " + losses + " GuessesLeft: " + guessesLeft + " Guesses so far: " + guessesSoFar + " Computer picked: " + letterToBeGuessed);
    }
